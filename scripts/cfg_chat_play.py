...
def main():
  ...
  history: List[Message] = [Message(Participant.System, misc_args.system_prompt)]
  ...
  while True:
    ...
    uncond_chat_to_complete: str = '\n'.join([
      *[
        f'{participant_names[participant]}: {message}'
        # skip system prompt
        for participant, message in history[1:]
      ],
    ...
    prompts: List[str] = [uncond_chat_to_complete, chat_to_complete]
    tokenized_prompts: TokenizerOutput = tokenizer(prompts, padding=False, add_special_tokens=False)
    ...
    try:
      ...
      prediction: LongTensor = model.generate(
        ...
        attention_mask=tensor(cond_mask, device=model.device).unsqueeze(0),
